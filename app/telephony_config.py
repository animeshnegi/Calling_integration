...
class TelephonyConfigSync:
    """Render DB-managed SIP objects into an internal Asterisk include file."""
    ...
    def _clean(value: Any) -> str:
        ...
        if "\n" in text or "\r" in text:
            raise ValueError("SIP configuration contains an invalid newline")
        return text
    ...
    def render(self) -> str:
        ...
        for provider in self.store.list_provider_details():
            ...
            lines.extend([
                ...
                *( [f"contact_user={did_user}"] if did_user else [] ),
                "retry_interval=30",
    ...
    def apply(self) -> None:
        content = self.render()
        fd, tmp_name = tempfile.mkstemp(prefix="pjsip.dynamic.", suffix=".conf", dir=str(self.path.parent))
        try:
