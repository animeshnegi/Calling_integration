...
class SettingsStore:
    ...
    def authenticate(self, username, password):
        with self._connect() as db:
            row = db.execute(
                "SELECT id,username,role,password_hash FROM admin_users WHERE username=? AND active=1",
                (username,),
            ).fetchone()
    ...
    def list_numbers(self):
        with self._connect() as db:
            rows = db.execute(
                "SELECT id,number,provider,description,inbound_extension,active FROM phone_numbers ORDER BY number"
            ).fetchall()
        return [dict(r) for r in rows]
...
def register_admin(app, config, on_telephony_change=None):
    ...
    def admin_state():
        return jsonify({
            ...
            "phone_numbers": store.list_numbers(),
