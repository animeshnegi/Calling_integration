...
Keys created by a customer account carry that account's immutable owner ID. Every resource response and action is constrained to it: extensions, numbers, calls, recordings, voicemail, and webhooks. Customer keys never return upstream SIP provider configuration. A platform key has system-wide scope. The authorization boundary is enforced by the API even if a customer supplies another account's extension, call ID, recording ID, mailbox, webhook ID, or number.
